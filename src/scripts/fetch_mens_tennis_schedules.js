...
const scheduleIds = require('../config/big12_mens_tennis');
require('dotenv').config();

// Function to fetch standings
async function fetchStandings() {
    try {
        const response = await axios.get('https://big12sports.com/standings.aspx?path=mten', {
            headers: {
...
async function main() {
    try {
        console.log('\n=== Big 12 Men\'s Tennis Schedule Fetcher ===\n');
        ...
        // Display standings
        console.log('\nCurrent Big 12 Men\'s Tennis Standings:');
        ...
        // Clear existing men's tennis entries from the database
        await knex('tennis_stats')
            .where('sport', 'mens-tennis')
            .del();
        
        console.log('\nCleared existing men\'s tennis data from database');
        ...
        for (const [team, scheduleId] of Object.entries(scheduleIds)) {
            ...
            // Save to database using the stringified schedule
            try {
        ...
        console.log('\n✅ Successfully updated all men\'s tennis team schedules');
        console.log('\nNOTE: Men\'s and women\'s tennis data is kept completely separate.');
